...
const ForumModule = (() => {
    ...
    function init() {
        ...
        style.innerHTML = `
            ...
            #forum-screen .comment-log {
                padding: 12px 0; border-bottom: 1px solid rgba(255,255,255,0.05); display: flex; flex-direction: column; gap: 4px;
            }
        ...
        document.addEventListener('keypress', function(e) {
            if (document.getElementById('forum-screen')?.classList.contains('active')) {
                if (e.key === 'Enter' && e.target.classList.contains('minimal-input')) {
                    addComment(e.target.id.split('-')[1]);
                }
            }
        });
        ...
        // 🌟 开启暗网心跳：每 3 秒扫描一次，如果有“时间到了”的新评论，自动重绘！
        setInterval(() => {
            const screen = document.getElementById('forum-screen');
            if (screen && screen.classList.contains('active')) {
                const now = Date.now();
                // 检查内存里是否有帖子的评论数量发生了“跨界”（隐藏变可见）
                let needRefresh = false;
                for (const p of _posts) {
                    // 如果存在一条评论，它的时间戳在刚才的3秒内心跳越过了现在，就说明有新评论该显示了
    ...
    async function _filterAndRender(type) {
        ...
        document.querySelectorAll('#forum-screen .inline-comments.expanded').forEach(el => {
            const pid = el.id.replace('fm-comments-', '');
            expandedIds.add(pid);
            const input = document.getElementById(`fm-input-${pid}`);
            if (input) inputValues[pid] = input.value;
        });
        ...
        // 🌟 3. 刷新后：神不知鬼不觉地恢复刚才的交互状态
        expandedIds.forEach(pid => {
            const section = document.getElementById(`fm-comments-${pid}`);
            const btn = document.getElementById(`fm-btn-msg-${pid}`);
            if (section) section.classList.add('expanded');
            if (btn) btn.classList.add('active');
            
            const input = document.getElementById(`fm-input-${pid}`);
            if (input) {
                if (inputValues[pid] !== undefined) input.value = inputValues[pid];
                if (_replyState[pid]) input.placeholder = `回复 ${_replyState[pid].name}...`;
            }
        });
        ...
        if (focusedInputId) {
            ...
            if (input) {
                ...
                input.value = ''; input.value = val; // 光标移到最后
            }
        }
    }
    ...
    function renderCommentsHTML(post) {
        ...
        // 🌟 给每条评论加上点击回复事件 (setReplyTarget)
        let commentsList = visibleComments.map(c => `
            <div class="comment-log" style="cursor:pointer; transition: background 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.02)'" onmouseout="this.style.background='transparent'" onclick="ForumModule.setReplyTarget('${post.id}', '${_escHtml(c.author)}', '${c.authorId || ''}')">
                <div class="clog-meta">
        ...
        let loadMoreHTML = '';
        if (pendingCount > 0) {
            loadMoreHTML = `<div style="text-align:center; padding: 12px 0 4px; border-top: 1px dashed rgba(255,255,255,0.05); margin-top: 8px;"><span style="font-family:var(--fm-font-mono); font-size:0.6rem; color:var(--fm-fg-muted); letter-spacing:2px; text-transform:uppercase;">[ <i class="ph-thin ph-asterisk" style="animation:spin 1s linear infinite; display:inline-block;"></i> INTERCEPTING SIGNAL... ]</span></div>`;
        } else if (visibleComments.length > 0) {
            loadMoreHTML = `<div style="text-align:center; padding: 12px 0 4px; border-top: 1px dashed rgba(255,255,255,0.05); margin-top: 8px;"><button id="fm-load-more-${post.id}" onclick="ForumModule.loadMoreComments('${post.id}')" style="background:transparent; border:none; font-family:var(--fm-font-mono); font-size:0.6rem; color:var(--fm-fg-muted); letter-spacing:2px; cursor:pointer; text-transform:uppercase; transition:0.2s;" onmousedown="this.style.color='#fff'" onmouseup="this.style.color='var(--fm-fg-muted)'">[ DECRYPT MORE LOGS... ]</button></div>`;
        }

        const placeholder = _replyState[post.id] ? `回复 ${_replyState[post.id].name}...` : "添加观测记录...";
        ...
        return `
            <div class="inline-comments" id="fm-comments-${post.id}">
                ...
                <div class="minimal-input-area">
                    ...
                    <input type="text" class="minimal-input" id="fm-input-${post.id}" placeholder="${placeholder}" autocomplete="off">
    ...
    // 🌟 新增：设置楼中楼回复目标
    function setReplyTarget(postId, authorName, authorId) {
        // 如果再次点击同一个人，取消回复
        if (_replyState[postId] && _replyState[postId].name === authorName) {
            _replyState[postId] = null;
            const input = document.getElementById(`fm-input-${postId}`);
            if (input) { input.placeholder = "添加观测记录..."; input.focus(); }
            return;
        }
        _replyState[postId] = { name: authorName, charId: authorId };
        const input = document.getElementById(`fm-input-${postId}`);
        if (input) {
    ...
    async function updateForumName(val) {
     ...
     async function publishPost() {
        ...
        let post = { id: 'fp_' + Date.now(), type, timestamp: Date.now(), likes: 0, comments:[], authorId: 'user', author: _forumProfile.name, avatarKey: _forumProfile.avatarKey };
    ...
    async function addComment(postId) {
        ...
        let authorName = _forumProfile.name || 'User';
        let avatarKey = _forumProfile.avatarKey || '';
        
        const replyInfo = _replyState[postId];

        // 🌟 核心：楼中楼身份切换逻辑
        if (isAnon) {
            authorName = '楼主';
            avatarKey = '';
        } else if (replyInfo && replyInfo.charId && replyInfo.charId !== 'npc' && replyInfo.charId !== 'user') {
            // 如果我们回复的是一个熟人角色，去查面具！
            try {
                const binding = await DB.bindings.get(String(replyInfo.charId)).catch(()=>null);
                ...
                const p = allPersonas.find(x => String(x.id) === String(personaId));
                if (p) {
                    authorName = p.name;
                    avatarKey = p.imgKey || '';
                }
            } catch(e) { console.error('读取角色面具失败', e); }
        }

        // 拼装楼中楼前缀
        const finalText = replyInfo ? `回复 ${replyInfo.name}：${text}` : text;
        ...
        try {
            ...
            setTimeout(() => {
                const section = document.getElementById(`fm-comments-${postId}`);
                const btn = document.getElementById(`fm-btn-msg-${postId}`);
                if (section) section.classList.add('expanded');
                if (btn) btn.classList.add('active');
            }, 50);

        } catch(e) {
            console.error('[ForumModule] Comment Error:', e);
            if(typeof Toast !== 'undefined') Toast.show('记录添加失败');
        }
    }
    ...
    async function deletePost(postId) {
        ...
        try {
            ...
            await DB.forum.del(postId); await loadPosts();
...
【返回格式】（严格 JSON，绝不输出其他废话）：
{
  "npcs":[
            ...
            if (result.npcs) {
                for (const npc of result.npcs) {
                    newComments.push({
                        ...
                        avatarKey: '',
                        text: npc.text,
                        // 点击 Load More 后的评论，在接下来的 5 ~ 45 秒内快速涌入
    ...
    return { init, onEnter, filterFeed, toggleComments, addComment, openPanel, closePanel, switchComposeType, publishPost, deletePost, changeForumAvatar, updateForumName, onEventImgSelected, removeEventImg,loadMoreComments,setReplyTarget, };
})();