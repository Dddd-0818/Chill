...
const UpdateModule = (() => {
  ...
  const CHANGELOG = [
  "✦ 教程在第二页的【索引】里，宝宝们有不懂的可以先看看。API的常见报错也在里面喔。",
  "✦ 一些小 Tips：主页可以上下滑动是正常的，没有遮挡的情况下不需要滑动。",
  "✦ 用户面具页（个人主页）点击顶部的 🔒 可以呼出/收起侧边栏；朋友圈页面是点击左上角按钮进行呼出和收起。",
  "✦ 主页底部栏的 🏠 按钮点击可以隐藏底部 Dock 栏，点击屏幕空白处可以再次召回。",
  "✦ 底部栏的 🔎 按钮是论坛，💬 气泡图标是聊天，最后一个人像图标是建立角色的地方。"
];
